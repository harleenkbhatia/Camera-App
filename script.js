...
vidRecBtn.addEventListener("click", function () {
    if (!recordState) {
        ...
        vidRecBtn.innerHTML = `<img src="https://img.icons8.com/flat-round/60/000000/stop.png"/>`;
    } else {
        mediaRecorder.stop();
        recordState = false;
        ...
        vidRecBtn.innerHTML = `<img src="https://img.icons8.com/flat-round/60/000000/record.png"/>`;
    }
});
...
function capture() {
    ...
    ctx.translate(canvas.width / 2, canvas.height / 2); //bringing our pen ctx to the center
    ctx.scale(zoom, zoom); //zooming
    ctx.translate(-(canvas.width / 2), -(canvas.height / 2)); //bringing it back
    ...
    // link.download = "img.png"; //img ka name
    // link.href = canvas.toDataURL(); //isse url bnjati h image lga ke
    // link.click();
}
...
function addFilterToScreen(filter) {
    ...
    filterScreen.style.height = videoPlayer.offsetHeight + "px"; //offset gives us actual height/width of the div container in px 
